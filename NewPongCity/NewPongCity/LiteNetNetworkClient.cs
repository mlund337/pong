...
namespace NewPongCity
{
    public class LiteNetNetworkClient
    {
        ...
        public void SnapshotHandler(ServerSnapshotPacket sp)
        {
            if (sp != null)
            {
                ...
                for (int i = 0; i < sp.Players; i++)
                {
                    ...
                    Console.WriteLine("Position of the player " + sp.Y[i]);
                    //update all players
        ...
        {
            _server = _client.Connect("192.168.156.47" /* host ip or name */, 9050 /* port */, "SomeConnectionKey" /* text key or NetDataWriter */);
            if (_server != null)
            {
                _isConnected = true;
        ...
        public void Update(GameTime gameTime,int y)
        {
